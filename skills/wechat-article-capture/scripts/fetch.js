...
import { writeFile, mkdir, stat, unlink, access } from 'fs/promises';
...
async function ensureWritableTmpdir() {
  const candidates = [
    process.env.CC_WECHAT_TMPDIR,
    join(homedir(), '.cc-connect', 'tmp'),
    join(homedir(), '.tmp'),
    process.env.TMPDIR,
    process.env.TEMP,
    process.env.TMP
  ...
  throw new Error('无法找到可写的临时目录供 Playwright 使用');
}
...
async function fetchWechatArticle(url, retries = 3, autoSavePath = null) {
  const tmpdir = await ensureWritableTmpdir();
